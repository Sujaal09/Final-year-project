<?php
    ...
    if (isset($_POST['add_room'])) {
        ...
        $values = [$frm_data['name'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['desc']];
        ...
        if ($stmt = mysqli_prepare($con, $q2)) {
            ...
            $flag = 0;
            die('Query was unable to prepare-insert');
        }

        if ($flag == 1) {
            echo "1";
        } else {
            echo "Error in adding room.";
        }
        exit();
    }

    elseif(isset($_POST['get_all_rooms']) && $_POST['get_all_rooms'] == '1') {
    ...
    elseif (isset($_POST['edit_room'])) {
        ...
        $values = [$frm_data['name'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc'], $frm_data['room_id']];
        if (update($q1, $values, 'siiiisi')) {
            $flag = 1;
        }
    
        $del_facilities = delete("DELETE FROM `room_facility` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    ...
    if(isset($_POST[ 'get_room_images'])){
        ...
        while($row = mysqli_fetch_assoc($res)){
            if($row['thumb']==1){
                $thumb_btn = "<i class='bi bi-check-lg text-light bg-success px-2p py-1 rounded fs-5'></i>";
            }else{
